...
import CallsGoing from './../components/pdf/CallsGoing';
...
export default function DocumentGenerator() {
    ...
    const {
        getAllReminders,
        getAllBeneficiariesWithDetails,
        getAllContactsWithDetails,
        getAllCalls, getAllCallsInGoing, getAllCallsOutGoing
    ...
    const [listCallsInGoing, setListCallsInGoing] = useState([]);
    const [listCallsOutGoing, setListCallsOutGoing] = useState([]);
    ...
    // *************** *********** ******************
    useEffect(() => {
        async function setGetAllCalls() {
            const response = await getAllCalls();
            setListCalls(response.data.data);
        }
        setGetAllCalls();
    }, []);

    useEffect(() => {
        async function setGetAllCallsInGoing() {
            const response = await getAllCallsInGoing();
            setListCallsInGoing(response.data.data);
            console.log(response);
            console.log(response.data.data);
        }
        setGetAllCallsInGoing();
    }, []);

    useEffect(() => {
        async function setGetAllCallsOutGoing() {
            const response = await getAllCallsOutGoing();
            setListCallsOutGoing(response.data.data);
            console.log(response);
            console.log(response.data.data);
        }
        setGetAllCallsOutGoing();
    }, []);
    ...
    return (
        <div id='documentGenerator' className='container-fluid'>
            <div className='input-group'>
                ...
                {/* Enlace para descargar el PDF */}
                <div id='downloadButtonContainer' style={{ width: '100%', marginTop: '20px' }}>
                    <PDFDownloadLink document={<Calls data={listCalls} />} fileName="listadoUsers.pdf"  >
                        {
                            ({ loading }) =>
                                loading ? <button id='downloadButton' className='btn btn-primary' >Loading document...</button> : <button id='downloadButton' className='mx-auto btn btn-primary'>Download now !</button>
                        }
                ...
                {/* Mostrar el documento*/}
                <PDFViewer style={{ width: '100%', height: '90vh', border: '5px solid black', borderRadius: '10px', marginTop: '20px' }}>
                    <Calls data={listCallsOutGoing} />
                </PDFViewer>
                {/* Enlace para descargar el PDF */}
                <div id='downloadButtonContainer' style={{ width: '100%', marginTop: '20px' }}>
                    <PDFDownloadLink document={<CallsGoing data={listCallsOutGoing} />} fileName="listadoUsers.pdf"  >
                        {
                            ({ loading }) =>
                                loading ? <button id='downloadButton' className='btn btn-primary' >Loading document...</button> : <button id='downloadButton' className='mx-auto btn btn-primary'>Download now !</button>
                        }
                ...
                {/* Mostrar el documento*/}
                <PDFViewer style={{ width: '100%', height: '90vh', border: '5px solid black', borderRadius: '10px', marginTop: '20px' }}>
                    <Calls data={listCallsInGoing} />
                </PDFViewer>
                {/* Enlace para descargar el PDF */}
                <div id='downloadButtonContainer' style={{ width: '100%', marginTop: '20px' }}>
                    <PDFDownloadLink document={<CallsGoing data={listCallsInGoing} />} fileName="listadoUsers.pdf"  >
                        {
                            ({ loading }) =>
                                loading ? <button id='downloadButton' className='btn btn-primary' >Loading document...</button> : <button id='downloadButton' className='mx-auto btn btn-primary'>Download now !</button>
                        }
